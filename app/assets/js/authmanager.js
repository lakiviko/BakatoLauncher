...
async function mojangUUID(nickname) {
    return fetch(`https://api.mojang.com/users/profiles/minecraft/${nickname}`)
            .then(data => data.json())
            .then(player => player.id);
}

function bukkitUUID(nickname) {
    var md5Bytes = crypto.createHash('md5').update("OfflinePlayer:" + nickname).digest()
 ...
 * Add a Plain account.
 * 
 * @param {string} username from microsoft.
 * @returns {Promise.<Object>} Promise which resolves the resolved authenticated account object.
 */
exports.addPlainAccount = async function(username) {
    let uuid = await mojangUUID(username).then(id => id ? id : bukkitUUID(username))

    const ret = ConfigManager.addPlainAccount(
        uuid,
        username
    )
    ConfigManager.save()
...
/**
 * Remove a Plain account.
 * 
 * @param {string} uuid The UUID of the account to be removed.
 * @returns {Promise.<void>} Promise which resolves to void when the action is complete.
 */
exports.removePlainAccount = async function(uuid){
    try {
        const authAcc = ConfigManager.getAuthAccount(uuid)
        if (authAcc) {
            ConfigManager.removeAuthAccount(uuid)
